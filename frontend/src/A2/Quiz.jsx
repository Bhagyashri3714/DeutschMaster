...
const QuizA2 = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/quiz/all?level=A2")
      .then((res) => {
  ...
  if (!questions.length) {
    return <div style={center}>No A2 Quiz Available</div>;
  }
  ...
  return (
    <div style={page}>
      <div style={card}>
        ...
        {isAdmin() && (
          <div style={adminBox}>
            <button onClick={() => navigate(`/a2/edit-quiz/${q.id}`)}>
              Edit
            </button>
          </div>
        )}
